...
def has_saved_wavelength_mapping(config: CalibrationConfig) -> bool:
    """Purpose: report whether calibration includes saved wavelength references. Rationale: startup and axis mode should stay in raw pixels until the user has saved a real wavelength map."""
    return bool(config.pixel_mapping_points) and config.wavelength_coefficients != DEFAULT_WAVELENGTH_COEFFICIENTS


def ensure_pixel_mode_without_mapping(config: CalibrationConfig) -> CalibrationConfig:
    """Purpose: reset empty wavelength mappings to pixel mode. Rationale: stale coefficient text alone should not make the app start in wavelength mode."""
    normalized = config.model_copy(deep=True)
    if has_saved_wavelength_mapping(normalized):
        return normalized
    normalized.wavelength_coefficients = list(DEFAULT_WAVELENGTH_COEFFICIENTS)
    return normalized
